...
- [~] M3.1 : `ModelRouter`/`src/llm_router.py`/`stage-model-assignment.yaml` **supprimés** ✅ · re-export `src/__init__.py` **nettoyé** ✅ · `router_advice` retargeté rôles natifs (`test_orchestrator_router_advice` vert) ✅ · `routes/routing_routes.py` : **dépréciation coordonnée signalée** ✅ (2026-07-04, commit 0d1fb9a) — headers RFC 8594 `Deprecation: true` + `Warning` 299 sur toutes les réponses `/api/route*` via dépendance router-level + log serveur unique, comportement inchangé, 4 tests · **`routes/routing_routes.py` + son test SUPPRIMÉS** ✅ (2026-07-05) — décision **(C) supprimer maintenant** : phase de construction, **0 utilisateur/consommateur REST externe** → pas de fenêtre de migration nécessaire ; le routeur n'était de toute façon **jamais monté** dans `app.py` (`/api/route*` non-live) — **CORRECTION 2026-07-05** : c'était FAUX, `app.py:803` montait bien `routing_router` → l'`import` supprimé a **cassé le boot de l'app** (`import app` levait `ModuleNotFoundError`) de `b4c56e4` jusqu'au fix `035d2f6` ; la suite ne l'a pas attrapé car aucun test n'importait le module `app` top-level → **test smoke boot subprocess ajouté** (`37a511c`, `tests/test_app.py::TestAppBoots`) · **reste** : `model-routing.json` (reclassé **PARTIEL** : garder la politique routing, retirer seulement le bloc `providers.*` une fois DB autoritaire — humain-gaté opérationnel, cf. ligne zen)
- [~] zen Go → `ModelEndpoint` natif — **slices 1-3 (2026-07-03)** : provider config dédupliquée (`_zen_provider_conn`/`_resolve_zen_endpoint_row`) ✅ · activation depuis endpoint natif derrière kill-switch `ODYSSEUS_ZEN_FROM_ENDPOINT` défaut-OFF (`zen_injection_enabled`/`zen_endpoint_registered`) ✅ · bloc `providers.opencode_zen` rendu optionnel ✅ · 14 tests · plan `docs/superpowers/plans/2026-07-03-zen-modelendpoint-migration.md` · ID modèle fantôme `deepseek-v4-flash-free` retiré (constante `_DEFAULT_ZEN_MODEL` = id confirmé, 4 sites, 3 tests) ✅ (2026-07-04, commit 48a79ee) · **audit chemin complet (2026-07-05)** : côté code = TERMINÉ (consumer `_zen_provider_conn` préfère la row DB · fallback JSON+env · kill-switch) ; l'**enregistrement de la row `ModelEndpoint` existe déjà nativement** via `do_manage_endpoints(action=add)` (`src/agent_tools/admin_tools.py:38`) → **pas de seed à écrire** (serait redondant, viol. native-first). **Reste = purement opérationnel + humain-gaté** : (a) un admin enregistre l'endpoint Zen via le tool natif `manage_endpoints` + pose `ODYSSEUS_ZEN_FROM_ENDPOINT=1`, (b) suppression finale du bloc `providers.*` une fois (a) fait. Aucun code manquant.
- [~] M3.2 : RRF existant réparé (fonction pure fusion vecteur+BM25) + câblé dans `search` live derrière kill-switch `ODYSSEUS_RRF_FUSION` (défaut OFF, blend 0.7/0.3 inchangé) ✅ · acontext réveillé via `MemoryProviderRegistry` (provider observe-only `on_session_end`, gate `ACONTEXT_ENABLED` défaut OFF, remplace le POST synchrone hardcodé du agent_loop) ✅ · Trinité : sémantique doc déléguée au `VectorRAG` natif, Graphify fantôme supprimé ✅ · **checkpoint câblé** ✅ (2026-07-04, commit a6a82ad) : `src/orchestrator/checkpoint_tracker.py` — bridge kill-switché `ODYSSEUS_CHECKPOINT` défaut-OFF, backend injectable, persisté au point MEMORY_OBSERVE post-round via primitive Obsidian `create_note`, best-effort ne lève jamais (9 tests)
- [~] M3.3 : observer consomme le SSE metrics + tool_events (`Observer.ingest_metrics`, câblé dans le bloc live, 8 tests) ✅ · **AUTOEVAL câblé** ✅ (2026-07-04, commit f96858d) : `src/orchestrator/autoeval.py` — kill-switch `ODYSSEUS_AUTOEVAL` défaut-OFF, `decide_keep_or_revert(verifier_reasons, drift_level)` (revert seulement sur raisons non-vides OU DriftLevel.HIGH), `apply_autoeval` avec `git_runner` injecté (revert destructif gated + mocké en test), consomme la sortie de `_run_verifier_subagent` (19 tests)
...
- [x] Token accounting unifié (1 writer + run_id de corrélation) (M3.X) ✅ (2026-07-04, commit 94f7cb2) : `run_id` de corrélation ContextVar partagé traces↔budgets↔metrics (commit 18c37f9) + comptabilité tokens unifiée derrière kill-switch `ODYSSEUS_UNIFIED_TOKENS` défaut-OFF (`unified_tokens_enabled`/`record_run_tokens`/`get_run_tokens` registre borné run-keyed, `accumulate_token_usage` réconcilie par run_id), 12 tests
- [x] M3.5 : enums `ChannelType.EMAIL`/`WEBHOOK` redondants supprimés (0 caller, 5 tests) ✅ · **adapters réveillés** ✅ (2026-07-04, commit 786c6de) : `src/channel_bootstrap.py` — Discord/Telegram réveillés in-process derrière kill-switches `ODYSSEUS_INPROCESS_DISCORD`/`ODYSSEUS_INPROCESS_TELEGRAM` défaut-OFF (`make_inbound_handler` routing inbound→handler natif, `deliver_outbound` delivery natif, `bootstrap_channels`), câblé dans `app.py` `_startup_event` via `_startup_channel_adapters` (20 tests)
- [~] Redondance résiduelle auditée contre `.planning/intel/INDEX.md §2` ✅ (2026-07-04) — **REDONDANT sûrs supprimés** : `ModelRouter`/`stage-model-assignment.yaml` (+ re-export `__init__.py` nettoyé), enums `ChannelType.EMAIL`/`WEBHOOK` · **DORMANT réveillés** : adapters Discord/Telegram via `event_bus.fire_event` natif (inbound) · **PARTIEL réparés en place** : `router_advice` retargeté rôles natifs, RRF/observer/autoeval branchés sur le natif, zen Go → `ModelEndpoint`. **Résiduels = déferrals tracés (préconditions explicites, pas des oublis)** : (1) `model-routing.json providers.*` — bloqué sur précondition DB-autoritaire ; (2) `routing_routes.py` suppression effective — déféré post-fenêtre-migration (dépréciation coordonnée déjà signalée) ; (3) `ChannelGateway` conservé comme abstraction outbound PARTIEL (redondance bus adressée via routing `fire_event`, classe non retirée) ; (4) **Obsidian READ/search non exposé à l'agent** — déféré délibérément (2026-07-05) : le leg WRITE est déjà live (`checkpoint_tracker.py:119` → `obsidian_mcp.create_note`, gate `ODYSSEUS_CHECKPOINT`), mais `obsidian_mcp.py` n'a pas le scaffold stdio MCP (`Server`/`@list_tools`/`@call_tool`) et n'est pas dans `_BUILTIN_SERVERS`. Exposer `list_notes`/`get_note`/`search_notes` est une **capacité agent net-new** (pas un wire-up) qui **exige un vault configuré** (défaut `/obsidian-vault` absent sur cette machine) + un design — à passer par BRAINSTORM, pas à câbler à l'aveugle
- [x] Zéro régression : base product intact avec kill-switch OFF ✅ (2026-07-04) — 4191 passed ; les 126 échecs sont **pré-existants/environnementaux** (tests `*_js` sans node, `gpu`/`hwfit` sans matériel, fixtures path-confinement) : sous-ensemble non-environnemental identique baseline `32ef455` (30) == HEAD (30) → **0 régression M3** ; 6 kill-switches confirmés OFF = comportement byte-identique ; 70 tests features + 150 tests sous-systèmes verts · **re-vérifié à HEAD `713d9cb` (2026-07-05)** : 4196 passed (+5 = tests round-trip channel), 123 failed + 2 errors **tous environnementaux/pré-existants** (liste complète catégorisée : `*_js` sans node, gpu/hwfit/macos-arm, symlink Windows, rg/npx/pip absents, fixtures path-confinement) — **aucun** dans channel_bootstrap/autoeval/orchestrator/memory (seul code modifié cette session) → **0 régression** ; la seule régression introduite en séance (assertion `test_e2e_smoke` sur enums EMAIL/WEBHOOK retirés) corrigée en `713d9cb`
```
