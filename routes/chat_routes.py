...
from core.database import ModelEndpoint, SessionLocal, get_session_mode, set_session_mode
...
def setup_chat_routes(
    ...
    async def chat_endpoint(request: Request, chat_request: ChatRequest) -> dict[str, str]:
        ...
        # Research injection
        research_blocked_by_policy = tool_policy.blocks("trigger_research") or tool_policy.blocks("manage_research")
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        workspace, workspace_rejected = _resolve_request_workspace(request, form_data.get("workspace"))
        ...
        active_email_ctx: dict[str, str] | None = None
        ...
        if active_email_uid:
            ...
            try:
                ...
                _ck = _read_cache_key(
                    active_email_account or None, active_email_folder, active_email_uid, owner=get_current_user(request)
                )
        ...
        try:
            ...
            _has_atts = bool(body and isinstance(body.get("attachments"), list) and body["attachments"]) or bool(
                form_data.get("attachments")
            )
        ...
        try:
            ...
            if not active_doc:
                ...
                active_doc = (
                    _owner_session_filter(_session_doc_q, ctx.user).order_by(DBDocument.updated_at.desc()).first()
                )
                if active_doc:
        ...
        if allow_web_search is not None and str(allow_web_search).lower() != "true" and not _explicit_web_intent:
        ...
        research_blocked_by_policy = bool(
            tool_policy.blocks("trigger_research") or tool_policy.blocks("manage_research")
        )
        effective_do_research = bool(do_research and _research_flags["do"] and not research_blocked_by_policy)
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                if not incognito:
                    ...
                    sess.add_message(
                        ChatMessage("assistant", full_response, metadata={"tool_events": [_ev], "model": sess.model})
                    )
                    session_manager.save_sessions()
                        ...
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                ...
                                if "delta" in data:
                                    ...
                                    last_metrics["model"] = (
                                        _reported_model or _actual_model or _answered_by or _requested_model
                                    )
                                    if ctx.context_length and last_metrics.get("input_tokens"):
                                        pct = min(
                                            round((last_metrics["input_tokens"] / ctx.context_length) * 100, 1), 100.0
                                        )
                            ...
                            if not last_metrics and full_response:
                                ...
                                _ctx_pct = (
                                    min(round((_est_in / ctx.context_length) * 100, 1), 100.0)
                ...
                _forced_tools = (
                    ...
                    if allow_web_search and str(allow_web_search).lower() == "true"
                    else None
                )
    ...
    async def rewrite_message(request: Request) -> StreamingResponse:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": (f"Here is the original response:\n\n{original_text}\n\nInstruction: {instruction}"),
            },
        ]
