    ...
    if account_id is None:
        ...
        if len(ids) <= 1:
            ...
            return await _auto_summarize_pass_single(
                days_back=days_back, account_id=(ids[0] if ids else None), progress_cb=progress_cb
            )
        outs = []
...
async def _auto_summarize_pass_single(days_back: int = 1, account_id: str | None = None, progress_cb=None) -> str:
    ...
    try:
        ...
        if not uid_list:
            _fb_uids, conn = _latest_inbox_fallback_uids(conn, lambda: _imap_connect(account_id, owner=account_owner))
        ...
        if auto_tag or auto_spam:
            if account_owner:
                _tag_existing = {
                    r[0]
                    for r in _c.execute("SELECT message_id FROM email_tags WHERE owner=?", (account_owner,)).fetchall()
                }
            else:
                _tag_existing = {
                    r[0]
                    for r in _c.execute("SELECT message_id FROM email_tags WHERE owner='' OR owner IS NULL").fetchall()
                }
        else:
            _tag_existing = set()
        _cal_existing = (
        ...
        try:
            _self_self_addr = (
                (_get_email_config(account_id, owner=account_owner).get("from_address") or "").strip().lower()
            )
        except Exception:
            _self_self_addr = ""
            ...
            try:
                ...
                if not need_sum and not need_reply and not need_class and not need_cal and not need_urgent:
                    ...
                    await _emit_progress(
                        progress_cb, f"Checked {examined}/{len(uid_list)} · {already_cached} already cached"
                    )
                    continue
                ...
                if need_sum:
                    ...
                    try:
                        ...
                        resp = await asyncio.to_thread(_req.post, url, json=payload, headers=req_headers, timeout=240)
                        if resp.ok:
                            rdata = resp.json()
                ...
                if need_reply:
                    ...
                    try:
                        ...
                        await _emit_progress(
                            progress_cb, f"Reply failed {_reply_failed} · checked {examined}/{len(uid_list)}"
                        )
                                    ...
                                    for op in ops[:3]:
                                        ...
                                        if action == "cancel":
                                            ...
                                            r = await do_manage_calendar(
                                                json.dumps({"action": "delete_event", "uid": cuid}), owner=_acct_owner
                                            )
                                            ...
                                            try:
                                                ...
                                                _phone_re = _re.compile(
                                                    r"(?:Phone|Tel|TEL|電話)[:：]?\s*(\+?[\d\s\-\(\)]{8,20})", _re.I
                                                )
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    alert_body = (
                                        ...
                                        f"Subject: {subject}\n"
                                        f"Date: {msg.get('Date', '')}\n\n"
                                        f"{body[:800]}" + ("..." if len(body or "") > 800 else "")
                                    )
                ...
                if need_class:
                    try:
                        ...
                        resp = await asyncio.to_thread(_req.post, url, json=payload, headers=req_headers, timeout=120)
                        if not resp.ok:
                            logger.warning(
