...
def _import_csv_contacts(text: str) -> dict:
    ...
    failed = 0
    total = 0
    existing_emails = {e.lower() for c in _fetch_contacts() for e in (c.get("emails") or []) if e}
        ...
        if ok:
            ...
            if phone:
                try:
                    ...
                    created = next(
                        (c for c in contacts if email.lower() in [e.lower() for e in c.get("emails", [])]), None
                    )
    ...
    if not _carddav_configured(cfg):
        ...
        for c in contacts:
            if c.get("uid") == uid:
                ...
                out.append(
                    _normalize_contact({"uid": uid, "name": name, "emails": emails, "phones": phones, "address": addr})
                )
                found = True
            else:
                out.append(c)
        if not found:
            out.append(
                _normalize_contact({"uid": uid, "name": name, "emails": emails, "phones": phones, "address": address})
            )
    ...
    async def add_contact(data: dict, _admin: str = Depends(require_admin)):
        ...
        if ok and phone:
            try:
                fresh = _fetch_contacts(force=True)
                created = next(
                    (c for c in fresh if name == c.get("name") and (not email or email in c.get("emails", []))), None
                )
                if created:
                    _update_contact(
                        created["uid"],
