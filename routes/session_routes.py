...
def setup_session_routes(session_manager: SessionManager, config: dict, webhook_manager=None):
    ...
    def list_sessions(request: Request):
        ...
        try:
            ...
            img_session_ids = set(
                r[0]
                for r in owner_filter(
                    db.query(GalleryImage.session_id).filter(GalleryImage.session_id != None), GalleryImage, user
                )
                .distinct()
                .all()
            )
            ...
            if model_to_use not in avail:
                ...
                for a in avail:
                    if _os.path.basename(a.rstrip("/")) == req_base:
                        ...
                        break
                if not found:
                    raise HTTPException(400, f"Model not found at server. Available: {', '.join(avail)}")
                model_to_use = found
    ...
    def delete_session(request: Request, sid: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail={"error": "SESSION_DELETE_ERROR", "message": "Failed to delete session"}
            )
    ...
    def list_archived_sessions(
        request: Request, search: str = "", offset: int = 0, limit: int = 20, sort: str = "recent", model: str = ""
    ):
    ...
    def create_session_openai(
        request: Request, name: str = Form("New Chat (OpenAI)"), model: str = Form("gpt-4o"), rag: str = Form(None)
    ):
    ...
    async def compact_session(request: Request, session_id: str):
        ...
        prior_compactions = sum(
            1 for m in history if _message_metadata(m).get("compacted") or "[Conversation summary" in _message_text(m)
        )
        prompt = SELF_SUMMARY_SYSTEM_PROMPT.replace("{count}", str(len(older))).replace(
            "{n}", str(prior_compactions + 1)
        )
        convo_text = "\n".join(f"{_message_role(m).upper()}: {_message_text(m)[:2000]}" for m in older)
        try:
