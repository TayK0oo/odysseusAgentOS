...
def _enforce_chat_privileges(request, sess) -> None:
    ...
    try:
        count = (
            ...
            .filter(_DbSess.owner == user, _Cm.role == "user", _Cm.timestamp >= _dt.utcnow() - _td(days=1))
            .count()
        )
    finally:
        db.close()
...
async def auto_name_session(session_manager, sess):
    ...
    try:
        ...
        t_url, t_model, t_headers = resolve_task_endpoint(sess.endpoint_url, sess.model, sess.headers, owner=owner)
        if not t_model:
...
def extract_preset(chat_handler, preset_id) -> PresetInfo:
    ...
    temperature, max_tokens, system_prompt, char_name = chat_handler.validate_and_extract_preset(preset_id)
    return PresetInfo(
...
async def preprocess(
    ...
    enhanced, user_content, text_ctx, yt_transcripts, att_meta = await chat_handler.preprocess_message(
        message,
        att_ids,
        sess,
...
def _has_auth_keys(headers) -> bool:
    ...
    return isinstance(headers, dict) and any(k.lower() in ("authorization", "x-api-key") for k in headers)
...
def _extract_thinking_meta(text: str) -> dict | None:
    ...
    if "<think>" in normalized:
        think_match2 = re.match(
            r"^[\s]*<think(?:ing)?>([\s\S]*?)</think(?:ing)?>\s*([\s\S]*)", normalized, re.IGNORECASE
        )
        if think_match2:
