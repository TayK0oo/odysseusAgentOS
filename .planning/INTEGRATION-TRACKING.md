...
| Composant | Vision | Statut | Preuve | Écart / action |
|---|---|---|---|---|
| Scrapling (web) | Brique web unique, MCP | 🔴 SCAFFOLD FAUX | container `:207` MAIS commande `python -m scrapling.mcp --port 8800` = **fausse** (vrai = `scrapling mcp --http --host 127.0.0.1 --port 8800`, streamable-HTTP ; sans `--http` = stdio, injoignable sur un port). `SCRAPLING_TOOLS` `tool_schemas.py:1442` invente `scrapling_fetch/spider/extract` = **aucun n'existe** (vrais outils : `get,bulk_get,fetch,bulk_fetch,stealthy_fetch,bulk_stealthy_fetch,screenshot,open_session,close_session,list_sessions`). Proxy REST `mcp_tools_routes.py` tape `/extract`+`/spider` = **endpoints inexistants**. `EXTERNAL_MCP_SERVERS` **jamais consommé** (grep src/ = défs seules) | **DESIGN-GATED** : (1) corriger cmd docker → `scrapling mcp --http` ; (2) enregistrer comme serveur MCP streamable-HTTP dans mcp_manager → vrais tools surfacent en `mcp__scrapling__*` ; (3) retirer SCRAPLING_TOOLS inventés + proxy REST cassé. **Non vérifiable sans le container tournant** (browser install requis) → ne pas câbler à l'aveugle |
| Kroki (diagrammes) | 25+ formats REST | 🟢 CÂBLÉ | tool agent `render_diagram` `agent_tools/diagram_tools.py` (encodage deflate+base64url réutilisé de `mcp_tools_routes.py:57`, retour image via `GENERATED_IMAGES_DIR` + `/api/generated-image/` → bulle image native). Registres parité : `FUNCTION_TOOL_SCHEMAS` + `BUILTIN_TOOL_DESCRIPTIONS` + `TOOL_HANDLERS`/`TOOL_TAGS` + dispatch `tool_execution.py`. Kill-switch `KROKI_ENABLED` (default on). Tests `tests/test_render_diagram_tool.py` (7) + parité | — |
