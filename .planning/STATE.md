...
Stopped at: **Cartographie native complète + re-plan M3 livrés.** 7 docs intel (`domains/01→07`) + `INDEX.md` (routing + redundancy map). `ROADMAP-M3-ORCHESTRATION.md` réécrit zéro-redondance. STATE à jour. M3.0 + M3.1 (advisory) déjà livrés (28 tests verts, commits 7370610, 84e209f, 3ded22d, f8fb55a).
Resume file: _(aucun)_

**Prochaine action :** exécuter le re-plan M3. Ordre le moins risqué : (1) **M3.1 cleanup** — retarget `router_advice` (retirer appel `ModelRouter`, garder `intent_gate` → rôles natifs), puis retirer re-export `src/__init__.py:3-4` AVANT de supprimer `llm_router.py`+configs fantômes+`routing_routes.py`. (2) **M3.4** (parallèle, sécurité pure) — fusionner command_validator+risk_classifier, couvrir run_script/run_local/api-shell. (3) **M3.2** — réparer le RRF orphelin (`rag_vector.py:749` bug) et le câbler à la place du blend 0.7/0.3. Voir `.planning/intel/INDEX.md §5` + `ROADMAP-M3-ORCHESTRATION.md §3`.
