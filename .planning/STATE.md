...
**Obsidian READ/search MCP livré** (2026-07-05, `ef23b78`) : scaffold stdio ajouté à `mcp_servers/obsidian_mcp.py` (Server/list_tools/call_tool), expose `list_notes`/`get_note`/`search_notes` (pas `create_note` — write leg = checkpoint natif), enregistré via `_optional_python_servers()` derrière gate `ODYSSEUS_OBSIDIAN_MCP` défaut-OFF (startup byte-identique), 7 tests. **Reste opérationnel** : configurer `OBSIDIAN_VAULT_PATH` (défaut `/obsidian-vault` absent) — dégrade proprement sinon.
