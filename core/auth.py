...
ADMIN_PRIVILEGES = {
    k: (True if isinstance(v, bool) else (0 if isinstance(v, int) else [])) for k, v in DEFAULT_PRIVILEGES.items()
}
...
def normalize_known_username(users: dict[str, Any], username: str | None) -> str | None:
...
class AuthManager:
    ...
    def delete_user(self, username: str, requesting_user: str) -> bool:
        ...
        with self._config_lock:
            ...
            try:
                ...
                if removed_tokens:
                    logger.info(f"Revoked {removed_tokens} API token(s) owned by deleted user '{username}'")
            except Exception:
        ...
        revoked = 0
        with self._sessions_lock:
            to_drop = [tok for tok, sess in self._sessions.items() if (sess or {}).get("username") == username]
            for tok in to_drop:
    ...
    def set_admin(self, username: str, is_admin: bool, requesting_user: str) -> SetAdminResult:
        ...
        with self._config_lock:
            ...
            if is_admin:
                ...
                target["privileges_before_admin"] = dict(target.get("privileges") or DEFAULT_PRIVILEGES)
                ...
                target["privileges"] = dict(target.pop("privileges_before_admin", None) or DEFAULT_PRIVILEGES)
    ...
    def validate_token(self, token: str | None) -> bool:
        ...
        with self._sessions_lock:
            ...
            elif session.get("username") not in self.users:
                self._sessions.pop(token, None)
                deleted_user = True
