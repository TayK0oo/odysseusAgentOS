...
_SSH_PATH_MEMBERS = ("/usr/bin", "/usr/local/bin", "/usr/local/cuda/bin", "/usr/lib/wsl/lib")
...
def _is_windows_bash_stub(path: str) -> bool:
    lowered = path.lower()
    return "system32\\bash.exe" in lowered or "sysnative\\bash.exe" in lowered or "windowsapps\\bash.exe" in lowered
    ...
    *,
    ...
    if strict_host_key_checking is not None:
        argv.extend(
            [
                "-o",
                "StrictHostKeyChecking=yes" if strict_host_key_checking else "StrictHostKeyChecking=no",
            ]
        )
