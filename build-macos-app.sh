...
# ── Icon (best effort) — center-crop docs/assets/odysseus.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/assets/odysseus.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/assets/odysseus.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/assets/odysseus.jpg" "$TMPIMG/sq.png"
  ...
  echo "  icon:        (skipped — no docs/assets/odysseus.jpg)"
fi
