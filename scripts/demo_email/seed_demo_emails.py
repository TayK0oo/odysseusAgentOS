...
def build_dataset() -> list[dict]:
    ...
    add(
        ...
        _msg(
            ...
            hours_ago=3,
            text=(
                "hi sweetie just checking did you eat today. you work too much on the computer. call me. love mom xoxo"
            ),
        ),
    )
...
def _wipe(conn: imaplib.IMAP4) -> int:
    ...
    if not safe_target and os.getenv("DEMO_ALLOW_WIPE") != "1":
        raise SystemExit(f"refusing to wipe non-demo target {USER}@{HOST}:{PORT} — set DEMO_ALLOW_WIPE=1 to override")
    typ, boxes = conn.list()
    n = 0
    names = []
    if typ == "OK":
