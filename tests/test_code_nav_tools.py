...
def _run(tool, content):
    # Accept a dict (serialized with json.dumps so Windows backslashes in
    # paths survive JSON decoding) or a pre-serialized JSON string.
...
def test_grep_ignore_case(repo):
    r = _run("grep", {"pattern": "needle", "ignore_case": True, "path": repo})
    assert "b.txt:2:" in r["output"]


def test_grep_glob_filter(repo):
    r = _run("grep", {"pattern": "needle", "ignore_case": True, "glob": "*.py", "path": repo})
    assert "a.py" in r["output"]
    assert "b.txt" not in r["output"]


def test_grep_no_match(repo):
    r = _run("grep", {"pattern": "zzzznotfound", "path": repo})
    assert r["exit_code"] == 0
...
def test_glob_literal_in_subdir(repo):
    ...
    r = _run("glob", {"pattern": "b.txt", "path": repo})
    assert r["exit_code"] == 0
...
def test_glob_multi_segment_single_star(repo):
    ...
    r = _run("glob", {"pattern": "sub/*.txt", "path": repo})
    assert r["exit_code"] == 0
    assert "b.txt" in r["output"]
...
def test_glob_star_does_not_cross_slash(repo):
    ...
    r = _run("glob", {"pattern": "sub/*.py", "path": repo})
    assert r["exit_code"] == 0
...
def test_read_file_offset_limit(repo):
    ...
    r = _run("read_file", {"path": p, "offset": 3, "limit": 2})
    assert r["exit_code"] == 0
