...
def schedule(tmp_path, monkeypatch):
    ...
    def _stored(sid):
        row = sqlite3.connect(db_path).execute("SELECT send_at FROM scheduled_emails WHERE id = ?", (sid,)).fetchone()
        return row[0]
...
async def test_naive_utc_send_at_unchanged(schedule):
    ...
    res = await endpoint({"to": "a@example.com", "body": "b", "send_at": naive}, owner="alice")
    assert res["success"] is True
