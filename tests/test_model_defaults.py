...
def _run_get_default_chat_test(monkeypatch, share_defaults_enabled, second_endpoint_only=False):
    ...
    global_settings = {
        ...
        "default_model_fallbacks": [{"endpoint_id": "fallback-ep", "model": "fallback-model"}],
    ...
    endpoints = [
        _FakeEndpoint(id="global-ep-123", base_url="http://global-endpoint:8000/v1", is_enabled=True),
        _FakeEndpoint(id="fallback-ep", base_url="http://fallback-endpoint:8000/v1", is_enabled=True),
    ]
...
def test_get_default_chat_user_no_prefs_share_enabled_resolves_global_defaults_fallbacks(monkeypatch):
    ...
    assert test_data["model"] == "qwen-3.6", "model should be resolved from global default_model"
