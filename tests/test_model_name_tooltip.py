...
def test_dropdown_item_has_title_tooltip():
    ...
    assert re.search(r"nameSpan\.title\s*=\s*m\.display", SRC), "dropdown model-name span needs a title tooltip (#1982)"
...
def test_header_indicator_has_title_tooltip():
    ...
    assert re.search(r"label\.title\s*=\s*modelId\b", body), "header model indicator needs a title tooltip (#1982)"
