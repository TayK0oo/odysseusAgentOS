...
class TestDeepSeekToolSupport:
    ...
    def test_deepseek_r1_7b_local_ollama_no_tools(self):
        ...
        assert result is False, (
            "deepseek-r1:7b on Ollama must not enable tool schemas (Ollama returns HTTP 400 for this model)"
        )
    ...
    def test_deepseek_r1_via_docker_no_tools(self):
        assert _compute_is_api_model("deepseek-r1:7b", "http://host.docker.internal:11434/v1") is False
    ...
    def test_gemma_local_ollama_defaults_to_fenced_tools(self):
        assert _compute_is_api_model("gemma4:e4b", "http://host.docker.internal:11434/v1") is False
    ...
    def test_qwen_native_ollama_defaults_to_fenced_tools(self):
        assert _compute_is_api_model("qwen3.5:4b", "http://localhost:11434/api/chat") is False
    ...
    def test_deepseek_cloud_api_gets_tools(self):
        ...
        assert result is True, "api.deepseek.com must be treated as tool-capable via _API_HOSTS"
    ...
    def test_endpoint_supports_true_overrides_blocklist(self):
        ...
        result = _compute_is_api_model("deepseek-r1:7b", "http://localhost:11434/v1", endpoint_supports=True)
    ...
    def test_endpoint_supports_true_overrides_ollama_default(self):
        ...
        native schemas."""
        result = _compute_is_api_model("qwen3.5:4b", "http://localhost:11434/v1", endpoint_supports=True)
        assert result is True

    def test_endpoint_supports_true_overrides_native_ollama_default(self):
        result = _compute_is_api_model("qwen3.5:4b", "http://localhost:11434/api/chat", endpoint_supports=True)
        assert result is True

    def test_endpoint_supports_true_overrides_gpt_oss_default(self):
        result = _compute_is_api_model("gpt-oss-20b", "http://localhost:8000/v1", endpoint_supports=True)
        assert result is True

    def test_endpoint_supports_false_overrides_cloud(self):
        """supports_tools=False on an endpoint gates even cloud APIs."""
        result = _compute_is_api_model("deepseek-chat", "https://api.deepseek.com/v1", endpoint_supports=False)
        assert result is False
