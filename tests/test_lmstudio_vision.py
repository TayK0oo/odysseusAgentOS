...
class TestLmStudioSupportsVision:
    ...
    PAYLOAD = {
        "models": [
            {
                "key": "qwen3.6-27b-custom-finetune",
                "architecture": "qwen35",
                "capabilities": {"vision": True, "trained_for_tool_use": True},
            },
            {"key": "text-only-llm", "architecture": "qwen35", "capabilities": {"vision": False}},
    ...
    def _serve(self, monkeypatch, payload):
        monkeypatch.setattr(chat_helpers.httpx, "get", lambda url, timeout=None: _FakeResponse(payload))
    ...
    def test_remote_endpoint_never_probed(self, monkeypatch):
        ...
        assert chat_helpers.lmstudio_supports_vision("https://api.openai.com/v1/chat/completions", "gpt-4o") is None
        assert calls["n"] == 0
...
class TestModelSupportsVision:
    ...
    def test_api_capability_overrides_name_heuristic(self, monkeypatch):
        ...
        assert (
            chat_helpers.model_supports_vision("qwen3.6-27b-finetune", "http://localhost:1234/v1/chat/completions")
            is True
        )
