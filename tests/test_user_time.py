...
def test_chat_preface_excludes_current_time_for_non_agent_chat():
    ...
    assert all(
        msg.get("role") != "system" or "## Current date and time" not in (msg.get("content") or "") for msg in preface
    )
...
def test_agent_system_prompt_includes_shared_current_time(monkeypatch):
    ...
    datetime_messages = [
        m for m in messages if m["role"] == "user" and "## Current date and time" in (m.get("content") or "")
    ]
