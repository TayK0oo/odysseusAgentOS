...
def test_do_functions_remain_async_through_shim():
    ...
    for name in _EXPECTED:
        if name.startswith("do_"):
            obj = getattr(ti, name)
            assert inspect.iscoroutinefunction(obj), f"{name} is not async via shim (got {type(obj).__name__})"
...
def test_every_facade_import_in_repo_resolves():
    ...
    assert not unresolved, f"facade consumers import names the shim does not re-export: {unresolved}"
