...
def _checkin_calendar_events(db, owner, start, end):
    ...
    """
    from core.database import CalendarCal as _CC
...
class TaskScheduler:
    ...
    async def start(self):
        ...
        try:
            ...
            try:
                ...
                stale = db.query(TaskRun).filter(TaskRun.status.in_(("running", "queued"))).all()
                if stale:
                    now = _utcnow()
                    for r in stale:
        ...
        try:
            ...
            try:
                ...
                for owner, n in groups:
                    ...
                    db.commit()
                    logger.warning(
                        "Default-assistant dedupe: owner=%r had %d rows, kept %s, dropped %d crew + %d orphan tasks",
                        owner,
                        n,
                        keep.id,
                        len(losers),
    ...
    async def _execute_task_locked(self, task_id: str, run_id: str, *, release_executing: bool = True):
        ...
        try:
            ...
            should_notify = (task.task_type or "llm") in {"llm", "research"} and getattr(
    ...
    CHECKIN_MCP_PATTERNS = [
        ...
        {"detect": "get_feed", "section": "RSS", "tool": "get_feed", "args": {}, "label_from_identity": False},
        {"detect": "list_feeds", "section": "RSS", "tool": "list_feeds", "args": {}, "label_from_identity": False},
        {
    ...
    async def _execute_checkin(self, task, crew, db, session_id: str, endpoint_url: str, model: str) -> str:
    ...
    async def _execute_llm_task(self, task, db) -> str:
            ...
            if self._session_manager:
                try:
                    self._session_manager.ensure_task_session(
                        session_id, f"[Task] {task.name}", endpoint_url, model, owner=task.owner, task=task
                    )
                except Exception:
                    pass
        ...
        try:
            ...
            if tool_idx:
                ...
                relevant_tools = compose_task_relevant_tools(rag_tools, ASSISTANT_ALWAYS_AVAILABLE, disabled_tools)
                logger.info(
                    f"[assistant] RAG selected {len(rag_tools)} tools + {len(ASSISTANT_ALWAYS_AVAILABLE)} always-available + shell/file defaults = {len(relevant_tools)} total for '{task.name}'"
                )
        except Exception as e:
    ...
    async def _deliver_task_result(self, task, result: str, db, model: str = None):
        ...
        if not session_id:
            ...
            if self._session_manager:
                try:
                    self._session_manager.ensure_task_session(
                        session_id, f"[Task] {task.name}", endpoint_url, model_name, owner=task.owner, task=task
                    )
                except Exception:
                    pass
