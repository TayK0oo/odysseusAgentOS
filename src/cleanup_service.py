...
async def archive_inactive_sessions(session_manager, owner: str | None = None) -> int:
    ...
    db = SessionLocal()
    try:
        q = db.query(DbSession).filter(DbSession.last_accessed < cutoff_date, DbSession.archived == False)
...
async def cleanup_old_sessions(session_manager, owner: str | None = None) -> tuple[int, float]:
    ...
    try:
        ...
        for session in sessions_to_delete:
            message_count = db.query(DbChatMessage).filter(DbChatMessage.session_id == session.id).count()
...
async def get_cleanup_preview(owner: str | None = None) -> dict[str, Any]:
    ...
    db = SessionLocal()
    try:
        archive_q = db.query(DbSession).filter(DbSession.last_accessed < cutoff_archive, DbSession.archived == False)
        ...
        for session in candidate_sessions:
            ...
            session_name_lower = session.name.lower() if session.name else ""
            matching_keywords = [
                keyword for keyword in CleanupConfig.PROTECTED_KEYWORDS if keyword in session_name_lower
            ]
