...
async def _drain(session_id: str, agen: AsyncGenerator[str, None], prev_task: asyncio.Task | None = None) -> None:
    ...
    try:
        ...
        _publish(
            run,
            f"event: error\ndata: {json.dumps({'error': 'Agent run failed before completion.', 'status': 500})}\n\n",
        )
