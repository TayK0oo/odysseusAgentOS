...
def get_search_config() -> dict[str, Any]:
    ...
    return {k: v for k, v in config.items() if not (isinstance(v, str) and _is_secret_key(k))}
...
def comprehensive_web_search(
    ...
    _source_list = [{"url": r.get("url", ""), "title": r.get("title", "")} for r in search_results if r.get("url")]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_url = {executor.submit(fetch_webpage_content, url, 8, retry_attempt=0): url for url in filtered_urls}
